...
export default function AssignSalesDialog({ 
  ...
  const assignSalesPerson = useMutation(api.sales.pipeline.mutations.assignSalesPerson);
  ...
  const handleConfirm = async () => {
    ...
    setIsPending(true);
    try {
      await assignSalesPerson({ clientId, salesPersonId: selectedUserId });
      toast.success(selectedUserId === "" ? "Lead unassigned" : "Lead assigned successfully");
      setIsOpen(false);
    } catch (error) {
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={(open) => {
      ...
      <AlertDialogContent className="rounded-2xl border max-w-sm ">
        <AlertDialogHeader>
          <AlertDialogTitle className="text-lg font-black tracking-tight">Assign Lead</AlertDialogTitle>
