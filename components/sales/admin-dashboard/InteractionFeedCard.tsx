...
import { Phone, Mail, Loader2, User, Building2 } from "lucide-react";
...
export function InteractionFeedCard() {
...
//   const [typeFilter, setTypeFilter] = useState("all");
  ...
  const { results, status, loadMore } = usePaginatedQuery(
    api.sales.admin.queries.getAdminInteractionsFeed,
    { 
      searchRep, 
      searchClient, 
      statusFilter,
    ...
    <Card className="flex flex-col shadow-sm border bg-card p-0">
      <CardHeader className="p-3 border-b border-border/50 bg-muted/30">
        <div className="flex items-center justify-between mb-3">
          ...
          <Badge className="bg-blue-500/10 text-blue-600 border-none text-[10px] font-semibold px-1.5">Calls & Emails</Badge>
        </div>
        ...
        <CardContent className="p-4 space-y-4">
          {results.length === 0 && status !== "LoadingFirstPage" ? (
            <div className="text-center py-10 text-muted-foreground text-xs font-semibold">
              {status === "LoadingMore" ? <Loader2 className="h-4 w-4 animate-spin mx-auto text-primary" /> : "No interactions"}
            </div>
          ) : (
            <>
              {results.map((item) => (
                <div key={item._id} className="relative pl-4 border-l-2 border-neutral-100 pb-4 last:pb-0">
                  <div className="absolute -left-2.25 top-0 h-4 w-4 rounded-full bg-white flex items-center justify-center border-2 border-neutral-100">
                    {item.type === 'call' ? (
                      ...
                      <Mail className="h-2 w-2 text-emerald-600" />
                    )}
                  </div>
                    ...
                    <div className="p-3 bg-muted/30 rounded-xl border border-border/50 group hover:border-border transition-all">
                      <div className="flex flex-col gap-2">
                        <p className="text-xs font-medium text-muted-foreground leading-relaxed">
                          {item.type === 'call' ? 'Call' : 'Email'} for <span className="font-semibold text-foreground">@{item.clientName}</span>
                        </p>
