...
interface AdminPipelineStageProps {
  status: Doc<"clients">["status"];
  count?: number;
}

export function AdminPipelineStage({ status, count }: AdminPipelineStageProps) {
  const [sortOrder, setSortOrder] = useState<"newest" | "oldest">("newest");
  const [search, setSearch] = useState("");

  const { results, status: queryStatus, loadMore } = usePaginatedQuery(
    api.sales.admin.queries.getAdminProspectsPaginated,
